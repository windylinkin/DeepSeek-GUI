...
把小K私人版从“随机 quick tunnel 临时连接”升级为“稳定 OpenAgent Local Connector 管理器”。目标不是重写 DeepSeek-GUI，也不是一次性做完整 AgentOS，而是在现有 exe 桌面版里增加一个长期可用的连接管理页面/模块，让林小逊以后只配置一次固定地址，ChatGPT 云端就能长期派任务到本地。
...
```text
D:\ai\openagent\scripts\start-openagent-chatgpt-connector-oauth.ps1
D:\ai\openagent\scripts\stop-openagent-chatgpt-connector.ps1
D:\ai\openagent\scripts\agent-task.ps1
```

`agent-task.ps1` 已支持：

```text
status / verify / verify-oauth / handoff / commit-check / teardown
```
...
4. 启动/停止本地 connector
5. 显示权限模式：read / read+write / read+write+run
...
先支持本地配置常量或 JSON 配置，不一定要做完整可编辑 UI。
...
D:\ai\openagent\scripts\start-openagent-chatgpt-connector-oauth.ps1
D:\ai\openagent\scripts\stop-openagent-chatgpt-connector.ps1
D:\ai\openagent\scripts\agent-task.ps1 -Task status
D:\ai\openagent\scripts\agent-task.ps1 -Task commit-check
D:\ai\openagent\scripts\agent-task.ps1 -Task handoff
D:\ai\openagent\scripts\agent-task.ps1 -Task verify
D:\ai\openagent\scripts\agent-task.ps1 -Task verify-oauth
```
...
- 本任务只解决长期连接管理，不做完整任务执行系统。
- 最终要能让林小逊在小K里看到固定 URL、OAuth 配置、权限模式和 connector 状态。
- 后续目标是：ChatGPT 云端稳定连接 MCP，直接通过 `oa_create_task` 派任务到本地队列。
